{
 ...
 Copyright © 2005-2008 Hars·nyi L·szlÛ (a.k.a. KeFear)
...
begin
  ...
  // ShowMessage( 'Kezdt¸k: '+TimeToStr(started)+ ' Befejezt¸k: ' + TimeToStr(finished) );
end;
...
begin
  ...
  begin
    ...
    //  if (not Matches('/*', seged)) and (Config.useadminname) { and (Config.adminname <> '') } then seged:= 'Å<' + Config.adminname + '> ' + seged;
    S := Cmd.Text;
...
begin
  ...
  // if ((Sender as TRichEdit).Lines.Count < ((Sender as TRichEdit).Height div ServerTab.Canvas.TextHeight('PR”BA') ) ) then//((Sender as TRichEdit).Height div (Sender as TRichEdit).Font.Size*0.9)-6 ) then
  // begin
   // SendMessage((Sender as TRichEdit).Handle, WM_VSCROLL, SB_BOTTOM, 0);
   // ShowMessage(inttostr((Sender as TRichEdit).Height div ServerTab.Canvas.TextHeight('PR”BA') ));
  // end;
...
begin
  ...
  begin
    ...
    //  IRCConsole.Lines.Add('sender: '+name+' ident: '+ident+' host: '+host+' k¸ldi: '+msg+' ezen kereszt¸l: '+channel)
  end;
end;
      ...
      try
        ...
        begin
          ...
          begin
            ...
            //        for i:=0 to params.Count-1 do ServerList[index].Memo.Lines.Add(params[i] + ' -> ' + values[i]);
            //       ServerList[index].Memo.Lines.Add('endfor vÈge');
            //       continue;
                    inif:= ifstate;
                 end;
...
begin
  ...
  begin
    ...
    AddARSSETimer(
      //        'PrÛba','script/proba.txt',1,3000,true,index);
...
begin
  ...
  //  Memo1.Lines.Add('Ide h˙ztuk: ' + inttostr(X)+ 'Ez lenne a szÈle?: '+ inttostr(ServerTab.ClientWidth));

  if ClickedTab <> DraggedTab then
  begin
    if ServerName.Visible then
      ServerName.Visible := false;
    //    Memo1.Lines.Add('Ez lett: ' + inttostr(clickedtab) + ' Ide h˙zva:' + inttoStr(DraggedTab));
    //    ShowMessage('Ez lett: ' + inttostr(clickedtab) + ' Ide h˙zva:' + inttoStr(DraggedTab));
    ...
    if ClickedTab < DraggedTab then
      ...
      begin
        ...
        //    Memo1.Lines.Add('Erre lett cserÈlve:' + inttostr(i+1) + '. '+ServerList[i+1].ServerName);
